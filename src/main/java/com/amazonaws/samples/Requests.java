...
public class Requests {
	...
	/**
	 * The submit method will create 1 x one-time t1.micro request with a maximum
	 * bid price of $0.03 using the Amazon Linux AMI.
	 *
	 * Note the AMI id may change after the release of this code sample, and it is
	 * important to use the latest. You can find the latest version by logging into
	 * the AWS Management console, and attempting to perform a launch. You will be
	 * presented with AMI options, one of which will be Amazon Linux. Simply use
	 * that AMI id.
	 */
	public void submitRequests() {
		...
		// t1.micro)
		// and the latest Amazon Linux AMI id available. Note, you should always use the
		// latest
		...
		// Setup an arraylist to collect all of the request ids we want to watch hit the
		// running
		// state.
		spotInstanceRequestIds = new ArrayList<String>();

		// Add all of the request ids to the hashset, so we can determine when they hit
		// the
		// active state.
	...
	/**
	 * The areOpen method will determine if any of the requests that were started
	 * are still in the open state. If all of them have transitioned to either
	 * active, cancelled, or closed, then this will return false.
	 * 
	 * @return
	 */
	public boolean areAnyOpen() {
		...
		// Create the describeRequest with tall of the request id to monitor (e.g. that
		// we started).
	...
	/**
	 * The cleanup method will cancel and active requests and terminate any running
	 * instances that were created using this object.
	 */
	public void cleanup() {
		// ==========================================================================//
